...
class Assistant(Agent):
    def __init__(self) -> None:
        super().__init__(
            ...
            'SambaNova is a hardware company that has developed a new chip called the RDU '
            '(reconfigurable data flow unit) that enables fast inference for AI.'
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    if TTS_PROVIDER.lower() == 'hume':
        tts = hume.TTS(
            ...
            description='The voice exudes calm, serene, and peaceful qualities, '
            'like a gentle stream flowing through a quiet forest.',
        )
    elif TTS_PROVIDER.lower() == 'elevenlabs':
        tts = elevenlabs.TTS(voice_id='ODq5zmih8GrVes37Dizd', model='eleven_multilingual_v2')  # type: ignore[assignment]
    else:

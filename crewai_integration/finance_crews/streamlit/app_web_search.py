...
# Main directories
finance_crews_dir = Path(__file__).parent.parent
crewai_integration_dr = finance_crews_dir.parent
repo_dir = crewai_integration_dr.parent

sys.path.append(str(finance_crews_dir))

from crews.crew_web_search.crew_web_search import WebSearchCrew
from utils.utilities import clean_markdown_content, convert_html_to_pdf, st_capture
...
def main() -> None:
    ...
    streamlit.markdown(
        ...
        <div class="kit-title">Financial Assistant - Web Search</div>
        """,
        unsafe_allow_html=True,
    )
